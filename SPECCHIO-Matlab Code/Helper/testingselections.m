...
allA = zeros(size(all_ids_A),1);
for i=0:size(allA)-1
    allA(i+1,1) = all_ids_A.get(i);
end


allB = zeros(size(all_ids_A),1);
...
time_FLAME                                                  = user_data.specchio_client.getMetaparameterValues(ids_FLAME, 'Acquisition Time');
